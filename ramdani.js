...
module.exports = Ramdani = async (Ramdani, mek) => {
	try {
        ...
        const listmsg = (from, title, desc, list) => { // ngeread nya pake rowsId, jadi command nya ga keliatan
            let po = Ramdani.prepareMessageFromContent(from, {"listMessage": {"title": title,"description": desc,"buttonText": "Pilih Disini","footerText": "© By NakataArdnta","listType": "SINGLE_SELECT","sections": list}}, {})
...
"externalAdReply": {
      ...
      const demoteAdmin = async function(to, target=[]){
          ...
          if (!packname) packname = 'Dibuat Oleh'; if (!author) author = '© NakataArdnta';author = author.replace(/[^a-zA-Z0-9]/g, '');	
     ...
     const time2 = moment().tz('Asia/Jakarta').format('HH:mm:ss')
       ...
       console.error(err)
        ...
        const ftoko = {key: {fromMe: false,participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "16505434800@s.whatsapp.net" } : {})},message: {"productMessage": {"product": {"productImage":{"mimetype": "image/jpeg","jpegThumbnail": fs.readFileSync(`./media/foto/thumb.jpg`)},"title": `© BY NakataArdnta`,"description": "NakataArdnta", "currencyCode": "IDR","priceAmount1000": "999999","retailerId": "Nakata Ardinata","productImageCount": 1},"businessOwnerJid": `0@s.whatsapp.net`}}}
...
danzrun = process.uptime() 
           Ramdani.setStatus(`${namabot} Aktif Selama ${(danzrun)} © Creator Nakata Ardinata`).catch((_)=>_); //DI UBAH YATIMM
...
> 532 ${prefix}nsfw_avatar`
               ...
               buttonsMessage = {
               contentText: `${allmenu}`,
               footerText: '© By NakataArdnta', imageMessage: imageMsg,
               buttons: buttons,
               headerType: 4
}
...
if (isBanned) return freply(mess.banned)
freply(`━━━━━ *INFO BOT* ━━━━━
❏ BASE ORI : Nakata Ardinata
❏ TYPE SC : nodejs
❏ DI BUAT PADA : sel, 29, mart, 2022
❏ GITHUB CREATOR : https://github.com/Nakataaard
❏ YT CREATOR : https://youtube.com/channel/UC2hMC3YSXSrDDhqOECXZkUg
*━━━━━━━━━━━━━━━━━━━━*`)
randommusic = fs.readFileSync(`./media/audio/${randomaudio}.mp3`)
Ramdani.sendMessage(from, randommusic, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
...
if (isBanned) return freply(mess.banned)
freply(`━━━━━ *SCRIPT* ━━━━━
❏ LINK VIA GITHUB : https://github.com/Nakataaard/V1
❏ BASE ORI : NakataArdinata
❏ TYPE SC : nodejs
❏ DI BUAT PADA : sel, 29, mart, 2022
*━━━━━━━━━━━━━━━━━━━━*`)
break
...
if (!isRegist) return freply(mess.regist)
if (isBanned) return freply(mess.banned)
freply(`*SILAHLAN HUBUNGI*\nhttp://wa.me/6285748894171?text=bang+mau+order+jasa+run+heroku:v`)
break
//JASA UP HEROKU
case 'jasaupgithub':
case 'jasaup':
if (!isRegist) return freply(mess.regist)
if (isBanned) return freply(mess.banned)
freply(`*SILAHLAN HUBUNGI*\nhttp://wa.me/6285748894171?text=bang+mau+order+jasa+up+github:v`)
break
//STORE MENU//
//TOP UP GAME
case 'topupgame2':
...
if (!isRegist) return freply(mess.regist)
if (isBanned) return freply(mess.banned)
gopeynya = 'https://telegra.ph/file/8fa93419975cb4db02e10.jpg'
teksnya = `*[ PAYMENT ]*
*Gopay :* 085748894171
*Dana :* 085748894171
*Ovo :* 085748894171
...
case 'tiktok':
              ...
              buttonsMessage = {footerText:'Jangan Lupa Subscribe Yt NahBotz-\n Help 100 subscriber', imageMessage: imageMsg,
       ...
       case 'img2url':
         ...
         case 'asupan':
              ...
              .then(async (body) => {
               ...
               sendMediaURL(from, `http://sansekai.my.id/ptl_repost/${asupanx}`, '\`\`\`ASUPAN BRO GUA DAPAT DARI Nakata Ardinata\`\`\`')
...
case 'ttp':
              if (!isRegist) return freply(mess.regist)
              if (isBanned) return freply(mess.banned)
              if (args.length == 0) return freply(`Example: ${prefix + command} Nakata Ardinata`)
              woy = args.join(" ")
              freply(mess.wait)
       ...
       case 'attp':
              if (!isRegist) return freply(mess.regist)
              if (isBanned) return freply(mess.banned)
              if (args.length == 0) return freply(`Example: ${prefix + command} NakataArdnta`)
              freply(mess.wait)
...
//------------------<18+ Menu>-----------------------   
       case 'randombokep':
              ...
              Ramdani.sendMessage(from, '*PERMINTAAN:* '+bokep+'\n*DOSA TANGGUNG PRIBADI*\n*NI BRO FREE BUAT KAMU DOWNLOAD SENDIRI:* '+ kep, text, { quoted: ftoko, caption: `NI BOKEP SAYA DAPAT DARI *© NakataArdnta* DOSA TANGGUNG SENDIRI🗿`})
              break
                case 'xnxx':
              if (!isRegist) return freply(mess.regist)
              if (isDewasa) return freply(mess.dewasa)
...
Ramdani.sendMessage(from, tapib1, document, { quoted: mek, filename:`NakataArdinata ~ 404 ${danz}.mp3`, mimetype: 'audio/application' })
...
"groupInviteMessage": {
"groupJid": "6285748894171-1328272333@g.us",
"inviteCode": "wFHwtOxGQN8OwK2x",
"inviteExpiration": "162533333338378",
"groupName": `NahBotz ~ 404 ${danz}`,
...
//------------------< Lainnya >-------------------
      ...
      case 'caripesan':
       ...
       case 'bugreport':
              ...
              Ramdani.sendMessage('6285748894171@s.whatsapp.net',`*Bug Report:* ${teks}`, text)
              break
       case 'readall':
              if (!isRegist) return freply(mess.regist)
              if (isBanned) return freply(mess.banned)
